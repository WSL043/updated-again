...
默认聊天使用 130 条项目规则和 89,856 条社区对话。聊天内容在浏览器内处理，不发送给外部服务；首次使用会从本站加载 2.49 MB 压缩语料。项目规则由 RiveScript 处理，其余问题使用本地检索。社区包包括 KdConv 中文对话和 ChatterBot Corpus 的 28 种语言。需要生成式回答时，访客可以手动连接 Puter，并使用自己账户的月度额度。网页不保存开发者 API Key。
