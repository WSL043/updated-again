...
export function ArchivePage({ station }: { station: UpdateStation }) {
  ...
  const window = paginate(filtered, page);
  const selected = window.items.find((entry) => entry.id === selectedId) ?? window.items[0];

  return (
    <section className="archive-page">
      <header className="page-heading">
        <div><p>共 {entries.length} 个版本</p><h1>更新记录</h1><span>公开版本都留在这里。回滚只动你这台机器，不会删账。</span></div>
        <button type="button" disabled={station.isBusy || !station.archive.history.length} onClick={station.rollback}>↶ 回滚最后一更</button>
      </header>

      <div className="archive-toolbar">
        <label htmlFor="archive-search">搜索版本</label>
        <input id="archive-search" type="search" value={query} onChange={(event) => { setQuery(event.target.value); setPage(1); }} placeholder="标题、日期、心情、类型……" />
        ...
        <div className="archive-projector">
          <div>
            ...
            <select id="edition-select" value={selected.id} onChange={(event) => setSelectedId(event.target.value)}>
              {window.items.map((entry) => <option key={entry.id} value={entry.id}>#{entry.sequence} · {entry.headline}</option>)}
            </select>
            <p>用当前搜索结果这一页的版本生成短片。</p>
          </div>
          <ReleaseFilm
      ...
      <nav className="archive-pagination" aria-label="更新记录分页">
        <button type="button" disabled={window.page === 1} onClick={() => setPage(window.page - 1)} aria-controls="archive-results">← 上一页</button>
        <span role="status">第 {window.page} / {window.pages} 页 · {window.start}–{window.end} / {window.total} 更</span>
        <button type="button" disabled={window.page === window.pages} onClick={() => setPage(window.page + 1)} aria-controls="archive-results">下一页 →</button>
      </nav>
      <div className="archive-layout">
        <div className="archive-list" id="archive-results">
          {window.items.map((entry) => <UpdateCard key={entry.id} entry={entry} installed={station.installedIds.has(entry.id)} onInstall={() => void station.installOne(entry.id)} busy={station.isBusy} />)}
